...
export default function FlightLogbook() {
  ...
  const aiInputRefNew = useRef(null);
  ...
  const totals = useMemo(() => {
    const t0 = {
      ...
      tos: 0, ldgs: 0, simTos: 0, simLdgs: 0,
    };
    const now = new Date();
    ...
    for (const e of filtered) {
      ...
      t0.simTos += (e.simTakeoffs || 0);
      t0.simLdgs += (e.simLandings || 0);
    }
    return t0;
  ...
  // ---- PUS logic — non-PUS fills copiXc; PUS sets 1 T/O + 1 LDG (if none yet) ----
  const applyPusLogic = (next) => {
    ...
    if (next.isPus) {
      ...
      next.copi = 0;
      next.copiXc = 0;
      // PUS flight = one segment: ensure exactly 1 T/O + 1 LDG when none entered yet
  ...
  const exportCsv = () => {
    const headers = [
      ...
      'takeoffsTotal','takeoffsNight','landingsTotal','landingsNight',
      'simTakeoffs','simLandings','remarks'
    ];
    const rows = entries.map((e) => [
      ...
      e.simTakeoffs || 0, e.simLandings || 0,
      (e.remarks || '').replace(/[\r\n]+/g, ' '),
    ]);
    const esc = (v) => {
      const s = String(v ?? '');
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          imported.push({
            ...
            simTakeoffs: parseInt(get('simTakeoffs'), 10) || 0,
            simLandings: parseInt(get('simLandings'), 10) || 0,
            remarks: get('remarks'),
          });
        }
...
SHEET COLUMNS (left → right) — column → JSON key mapping:
  ...
  FE                  → fe
  OTHER               → other
DUTY CODES (seen across this airline's sheets — but ALWAYS verify against which columns actually contain time):
  CO    Co-pilot line flight   → copi = BLOCK; copiXc = X-C value; copiNight = NIGHT value.
  OBC   On-board / co-pilot    → treat the same as CO: copi = BLOCK; copiXc = X-C; copiNight = NIGHT.
  OB    Observation / training → usually NO block time is logged as copi. Put BLOCK into "other"; copy any X-C into otherXc; put a note "OB" in remarks. If OJT column has time, that's instructor time (see below).
  COX   Co-pilot extra / non-line → flightTime = BLOCK AND other = BLOCK (same value). DO NOT set copi. The X-C column for COX rows is IGNORED (all *Xc = 0:00).
  PUS   Pilot-under-supervision → pus = BLOCK; pusXc = X-C; pusNight = NIGHT.
  CAPT  Captain                → pic = BLOCK; picXc = X-C; picNight = NIGHT.
  SOLO  Solo                   → sic = BLOCK; sicXc = X-C; sicNight = NIGHT.
  DUAL  Dual instruction       → dual = BLOCK; dualXc = X-C; dualNight = NIGHT.
  DH    Dead-head (passenger)  → SKIP this row entirely. Do NOT output it.
  M11 M12 M21 M22  Simulator/flight-training session → sim = the time shown (often in the SIM column or BLOCK); simImc = round(sim * 0.75); flightTime = 0; from/to/flightNo may be empty. Put the M-code in remarks.

GENERAL RULE (most important): The DUTY code tells you the pilot's role, but you MUST confirm by looking at WHICH numeric column actually has a value on that row, then route the BLOCK time to the matching JSON field:
  - value under COPI column   → copi
  - value under CAPT column   → pic
  - value under SOLO column   → sic
  - value under PUS column    → pus
  - value under DUAL column   → dual
  - value under OTHER column  → other
  - value under X-C column    → the *Xc that matches the duty (copiXc for CO/OBC, pusXc for PUS, etc.); for COX leave all *Xc = 0:00
  - value under NIGHT / COP NIGHT / CAP NIGHT → copiNight / picNight as appropriate
  - value under IMC / HOOD     → imc / hood
  - value under OJT, OBS, CKE, CKR, CKR JS, R/I, R/I JS → fe (examiner/checker/observer); value under F/I, F/I JS → fi. Append the code to remarks.
  - value under SIM columns    → sim, then simImc = round(sim*0.75)
When a DUTY code is unfamiliar, IGNORE the code name and route purely by which column holds the time. Always copy the raw DUTY code text into "remarks".

TIME FORMAT: cells use "HH+MM" (e.g. "01+14" = 1h14m). Convert to "HH:MM" on output. Empty cell → "0:00". Skip the "合計" (sum) row at the bottom and any monthly-summary tables below the flight table.
...
JSON SHAPE (every key must be present in every object):
{
  ...
  "simTakeoffs":0,"simLandings":0,
  "remarks":"string (include the DUTY code)"
}

LANDINGS/TAKE-OFFS: For every real flight row (CO/OBC/OB/COX/PUS/CAPT/SOLO/DUAL), takeoffsTotal=1, landingsTotal=1, simTakeoffs=0, simLandings=0. For SIM rows (M11/M12/M21/M22): takeoffsTotal=0, landingsTotal=0; if the sheet shows landing/take-off counts for the SIM session (often written in parentheses), put them in simTakeoffs / simLandings, otherwise 0. For DH rows: row is skipped.
NIGHT LANDING: if NIGHT + CAP NIGHT + COP NIGHT for the row is > 0, set takeoffsNight=1 and landingsNight=1. Otherwise 0.
...
Final reminder: return ONLY the JSON array, nothing else.`;
      ...
      const made = arr.map((r) => {
        ...
        return {
          ...
          simTakeoffs: parseInt(r.simTakeoffs, 10) || 0,
          simLandings: parseInt(r.simLandings, 10) || 0,
          remarks: r.remarks || '',
        };
      });
  ...
  const ChartRow = ({ def }) => {
    ...
    if (def.kind === 'tkof' || def.kind === 'ldg') {
      ...
      const setCount = (n) => {
        const wasNight = items.length > 0 && items.every((it) => it.night);
        const next = [];
        for (let i = 0; i < n; i++) {
          // preserve existing night flag per slot; new slots inherit the row's night state
          next.push(items[i] ? { ...items[i] } : { night: wasNight });
        }
        setItems(next);
      };
      // Whole-row night: checked when there is at least 1 and all are night
      const allNight = items.length > 0 && items.every((it) => it.night);
      const setRowNight = (on) => setItems(items.map((it) => ({ ...it, night: on })));
      const Icon = def.kind === 'tkof' ? PlaneTakeoff : PlaneLanding;
      const nightPal = pal('night');
      const simField = def.kind === 'tkof' ? 'simTakeoffs' : 'simLandings';
      const simVal = form[simField] || 0;
      const showSim = (form.sim || 0) > 0;
      const setSim = (n) => set(simField, Math.max(0, n));
      return (
        <div className="border border-slate-700/60 rounded-lg overflow-hidden bg-slate-900/30">
          <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
            ...
            {/* Night checkbox (whole row) */}
            <button type="button" onClick={() => setRowNight(!allNight)}
              ...
              <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center ${
                allNight ? `${nightPal.chip} border-transparent` : 'border-slate-500'
              }`}>
                {allNight && <span className="text-[9px] text-slate-900 font-bold leading-none">✓</span>}
              </span>
              {t.nightFlag}
            </button>
          </div>

          {/* SIM count — only when simulator time is logged on this entry */}
          {showSim && (
            <div className="flex items-center gap-2 px-3 pb-2.5 pt-0.5 border-t border-slate-800/60 bg-slate-950/30 flex-wrap">
              <div className="flex items-center gap-1.5 flex-1 min-w-0 pl-4">
                <span className="text-slate-700">└</span>
                <span className={`w-1.5 h-3 rounded-sm ${pal('sim').chip}`}></span>
                <span className={`text-[11px] font-medium truncate ${pal('sim').text}`}>{t.simLdgLabel} ( )</span>
              </div>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => setSim(simVal - 1)}
                  className="w-7 h-7 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-base leading-none">−</button>
                <span className="font-mono text-amber-300 w-8 text-center">({simVal})</span>
                <button type="button" onClick={() => setSim(simVal + 1)}
                  className="w-7 h-7 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-base leading-none">+</button>
              </div>
            </div>
      ...
      case 'tkof':    return (
        <span className="font-mono text-white">
          ...
          {(e.takeoffs?.filter(x => x.night).length || 0) > 0 &&
            <span className={`${pal('night').text} ml-1`}>(N)</span>}
          {(e.simTakeoffs || 0) > 0 &&
            <span className={`${pal('sim').text} ml-1`}>({e.simTakeoffs})</span>}
        </span>
      );
      case 'ldg':     return (
        <span className="font-mono text-white">
          ...
          {(e.landings?.filter(x => x.night).length || 0) > 0 &&
            <span className={`${pal('night').text} ml-1`}>(N)</span>}
          {(e.simLandings || 0) > 0 &&
            <span className={`${pal('sim').text} ml-1`}>({e.simLandings})</span>}
        </span>
      );
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {CARD_DEFS.filter(c => visibleCards.includes(c.id)).map((c) => {
                  let value;
                  if (c.id === 'totalTkof') {
                    value = totals.simTos > 0 ? `${totals.tos} (${totals.simTos})` : String(totals.tos);
                  } else if (c.id === 'totalLdg') {
                    value = totals.simLdgs > 0 ? `${totals.ldgs} (${totals.simLdgs})` : String(totals.ldgs);
                  } else if (c.isCount) {
                    value = String(totals[c.key]);
                  } else {
                    value = fmtHHMM(totals[c.key]);
                  }
                  return (
          ...
          <div className="bg-slate-900/40 border border-slate-700/60 rounded-xl overflow-hidden">
            <div className="px-4 sm:px-5 py-3 border-b border-slate-700/60 bg-slate-900/60 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                {!editingId && (
                  <>
                    <button onClick={() => aiInputRefNew.current?.click()} disabled={parsing}
                    ...
                    <input ref={aiInputRefNew} type="file" accept="image/*,application/pdf" className="hidden"
